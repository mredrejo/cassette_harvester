...
N16961_superIn[(nrow(N16961_superIn)+1),] <- c("NC_002506.1", 
                                               "Cassette",
                                               as.numeric(N16961_superIn[N16961_superIn$GeneID=="attc_176",4]) + 1, 
                                               435387,"+")
...
table_counts2 <- data.frame(cbind(row.names(counts$counts), counts$counts, N16961full$Chr, N16961full$Start,N16961full$End))
table_counts$normalized <- as.numeric(table_counts$mapped_GCF_000006745_reads_chopped_filtered_minimap.bam)/72108
...
names(table_counts_final) <- c("Chromosome","Feature_name","Start","Stop","Strand","Type","Counts","Normalized","Gene","Bakta Annotation")



#write table of counts 
write.table(table_counts_final,"../results_GCF_000006745/coverage_N16961_IF2_nofrac.csv", quote=FALSE,sep=";",row.names = FALSE)

#write table of counts within superintegron
write.table(table_counts_final[as.numeric(table_counts_final$Start)>309750 & as.numeric(table_counts_final$Stop)<435387 & table_counts_final$Chromosome=="NC_002506.1",],"../results_GCF_000006745/coverage_superintegron_nofrac.csv", quote=FALSE,sep=";",row.names = FALSE)

#ratio CDS mapping
mean(as.numeric(table_counts_final$Counts[as.numeric(table_counts_final$Start)>309750 & as.numeric(table_counts_final$Stop)<435387 & table_counts_final$Type=="cds" & table_counts_final$Chromosome=="NC_002506.1"])) / 
  mean(as.numeric(table_counts_final$Counts[table_counts_final$Chromosome=="NC_002505.1" & table_counts_final$Type=="cds"| as.numeric(table_counts_final$Start)<309750 & as.numeric(table_counts_final$Stop)>435387 & table_counts_final$Type=="cds" & table_counts_final$Chromosome=="NC_002506.1"]))
...
ggsave("../results_GCF_000006745/counts_chromosomes.pdf", plot=counts, width=8,height=5)
ggsave("../results_GCF_000006745/counts_chromosomes_log10.pdf", plot=counts+ scale_y_log10()+ylab("Log10(Counts)"), width=8,height=5)

#plot counts/Cassette
casettes <- ggplot(data=table_counts_final[table_counts_final$Type=="cassette",]) +
  geom_point(aes(x=reorder(Feature_name,as.numeric(Start)),y=(as.numeric(Counts)), colour=ifelse(as.numeric(Start)>309750 & as.numeric(Stop)<437699 & Chromosome=="Chr_2" ,"Superintegron","Chromosome"))) +
  facet_grid(~Chromosome,scales="free")+ theme_classic() +
  scale_x_discrete(expand = c(0.01, 0),breaks = table_counts_final[table_counts_final$Type=="cassette",]$Feature_name[rep(c(T,F,F,F))]) +
...
ggsave("../results_GCF_000006745/counts_cassettes_integron.pdf", plot=casettes, width=6,height=5)
ggsave("../results_GCF_000006745/counts_cassettes_integron_log10.pdf", plot=casettes+ scale_y_log10()+ylab("Log10(Counts)"), width=6,height=5)
...
#write table
write.table(multimap[multimap$V2=="Assigned" & multimap$V3>0,],"../results_GCF_000006745/multimapped_N16916_final.csv", quote=FALSE,sep=";")
...
spurious <- alignments[alignments$V1=="NC_002505.1" | alignments$V1=="NC_002506.1" & alignments$V2 < 309750 | alignments$V1=="NC_002506.1" & alignments$V2 >435387, ]
...
writeLines(onlyoutside,"../results_GCF_000006745/spurious_reads_onlyoutside.txt")
...
write.table(spurious_only_GTF,"../results_GCF_000006745/spurious_ONLY_insertion_sites_Q30_100.gtf",col.names = FALSE,row.names = FALSE,quote=FALSE,sep="\t")
...
kk <- cbind(table_counts_final[table_counts_final$Type=="cassette",c(2,7)], N16961_superIn[N16961_superIn$type_elt=="attC",7])
kk$length <- as.numeric(table_counts_final[table_counts_final$Type=="cassette",4])-as.numeric(table_counts_final[table_counts_final$Type=="cassette",3])
kk$full <- as.numeric(table_counts_final[table_counts_final$Type=="cassette",7]) 

ggscatter(kk, x = "full", y = "evalue", color = "#CC0000", shape = 21, size = 2,
                    add = "reg.line", add.params = list(color = "#CC0000", size = 2), conf.int = TRUE, 
                   cor.coef = TRUE, cor.method = "pearson") + yscale("log10", .format = TRUE) + 
  xlab("Cassette Count") + ylab("AttC e-value")
ggsave("../results_GCF_000006745/cor_evalue_count.pdf",  width=5,height=5)

## plots with evalue fill gradient

cassettes2 <- ggplot(data=kk) +
  geom_point(aes(x=reorder(Feature_name,as.numeric(row.names(kk))),y=(as.numeric(Counts)), colour="black", fill=log10(evalue)),shape=21,size=3) +
  theme_bw() + scale_fill_gradient2() +
  scale_x_discrete(expand = c(0.01, 0),breaks = table_counts_final[table_counts_final$Type=="cassette",]$Feature_name[rep(c(T,F,F,F))]) +
  theme(axis.text.x=element_text(angle=90,hjust = 1),axis.text.y=element_text(size=12,face="bold"), legend.position = "none") +
  ylab("Counts") + xlab("") + labs(color="")

ggsave("../results_GCF_000006745/counts_cassettes_integron_evalues.pdf", plot=cassettes2, width=6,height=5)
ggsave("../results_GCF_000006745/counts_cassettes_integron_log10_evalues.pdf", plot=cassettes2+ scale_y_log10()+ylab("Log10(Counts)"), width=6,height=5)
